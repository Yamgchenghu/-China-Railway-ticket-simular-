...
while True:
    try:
        ...
        try:#强制展示base.png
            try:
                if data[54]!='0' and data[54]!='0\n':
                    base_img = pygame.image.load(resource_path(f"picts/base{data[54]}.png"))
                    screen.blit(base_img, (0, 0))
            except:
                base_img = pygame.image.load(resource_path("picts/base1.png"))
                screen.blit(base_img, (0, 0))
        except:
            pass

        dlc.moutput(screen,"#p0023 0063 0031 0037 0039 0032 0031 0036 0032 0035 0035 0023 0070 0030 0030 0034 0032 0020 0037 0041 0044 0039 0020 0030 0030 0033 0043 0020 0037 0044 0032 0042 0020 0036 0046 0037 0043 0020 0030 0030 0033 0045 0020 0037 0032 0045 0043 0020 0037 0041 0043 0042 0020 0035 0032 0033 0036 0020 0034 0046 0035 0043 0020 0046 0046 0030 0043 0020 0035 0042 0038 0043 0020 0035 0031 0036 0038 0020 0035 0031 0034 0044 0020 0038 0044 0033 0039 0023#",(100,100,100),(270-243//2,280))
        ...
        if page == "M3":
            ...
            if data[54]!="3" and data[54]!="3\n":
                ...
                dlc.moutput(screen,"#B开源版本，请勿倒卖",(100,100,100),(10,140),num=1)
        if page == "GFMF":
            dlc.moutput(screen,"#H编辑车次  #H编辑票面  #B其他信息",(100,100,100),(10,10))
            pygame.draw.rect(screen,(0,128,168),(0,35,550,2))
        ...
        for e in pygame.event.get():
            ...
            if e.type==pygame.KEYDOWN:
                if page=="GFMF":
                    ...
                    if e.key>=pygame.K_0 and e.key<=pygame.K_9:
                        ...
                        if e.key>=pygame.K_0 and e.key<=pygame.K_9:
                            ...
                            if data[7]!="":
                                ...
                                x=e.key-pygame.K_a
                                if len(data[7])<10 and x<=1:
                                    data[7]+=chr(x+65)
                                elif x<=26 and x in list_num_ci:
                                    list_num_ci=[pygame.K_q,pygame.K_w,pygame.K_e,pygame.K_r,pygame.K_t,pygame.K_y,pygame.K_PERIOD,pygame.K_SPACE]#小数点
                                    input_check_in=["一","二","三","楼","检票口","候车室","."," "]##QWERTY输入第一行信息
                                    data[7]+=input_check_in[list_num_ci.index(e.key)]
                                else:
                                    dlc.addts("数据超限",color=(240,100,20))
                elif page=="CC":
                    tmp_CC_ZM=[pygame.K_a,pygame.K_c,pygame.K_d,pygame.K_k,pygame.K_t,pygame.K_z,pygame.K_g,pygame.K_s,pygame.K_x,pygame.K_j,pygame.K_l,pygame.K_y]#ACDKTZGSXJLY
                    if e.key==pygame.K_BACKSPACE:
                        if data[15]!="":
                            data[15]=data[15][:-1]
                    elif e.unicode.isdigit() or e.unicode.isspace() or e.key in tmp_CC_ZM:
                        if len(data[15])<7:
                            if e.key in tmp_CC_ZM:
                                if e.key!=pygame.K_j:
                                    data[15]=chr(e.key-pygame.K_a+65)
                                else:
                                    data[15]+="J"
                            else:
                                data[15]+=e.unicode
                    ...
                    cin_temp_key=[pygame.K_q,pygame.K_w,pygame.K_e,pygame.K_r,pygame.K_t,pygame.K_y,pygame.K_u,pygame.K_a,pygame.K_b,pygame.K_c,pygame.K_d,pygame.K_f]#qwertyu abcdf
                    if e.key in cin_temp_key:
                        data[25]+=cin_temp[cin_temp_key.index(e.key)]
                elif page=="PM-KS3":
                    if e.key==pygame.K_BACKSPACE:
                        if sfxx_pmks3[0]!="":
                            sfxx_pmks3[0]=sfxx_pmks3[0][:-1]
                    elif e.unicode.isdigit() and len(sfxx_pmks3[0])<18:
                            sfxx_pmks3[0]+=e.unicode
                elif page=="PM-KD":
                    if e.key==pygame.K_BACKSPACE:
                        if sfxx_pmks6[0]!="":
                            sfxx_pmks6[0]=sfxx_pmks6[0][:-1]
                    elif e.unicode.isdigit() and len(sfxx_pmks6[0])<14:
                            sfxx_pmks6[0]+=e.unicode
                    elif e.key>=pygame.K_a and e.key<=pygame.K_z:
                            x=e.key-pygame.K_a
                            if len(sfxx_pmks6[0])<14:
